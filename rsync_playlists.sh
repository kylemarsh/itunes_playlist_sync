...
#    /usr/bin/rsync --server -vlogDtprc . /path/to/remote/playlist/directory
...
#         command="/usr/bin/rsync --server -vlogDtprc . /path/to/remote/playlist/directory",no-agent-forwarding,no-port-forwarding,no-pty,no-user-rc,no-X11-forwarding 

SOURCE_DIR="/tmp/playlists/"  # The trailing / is important here
TARGET_DIR="/path/to/remote/playlist/directory"
IDENTITY_FILE="$HOME/.ssh/playlist-rsync.id_rsa"
REMOTE_USER="username"
REMOTE_HOST="hostname"
rsync -ac --rsync-path /usr/bin/rsync -e "ssh -i $IDENTITY_FILE" $SOURCE_DIR $REMOTE_USER@$REMOTE_HOST:$TARGET_DIR
